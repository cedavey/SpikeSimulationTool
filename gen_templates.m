...
[templates.abs_refract_time, templates.abs_refract_index] = refract_period(tInit, xInit, const, duration, initial_ap, sampling_rate);

% Calculate when second input will generate same output as previous output
% and generate transition templates
[templates.end_time, templates.end_index, templates.transition] = gen_trans(tInit, xInit, const, duration, initial_ap, sampling_rate, templates);

% Input function
Iapp = @(t) 10*exp(-((t - initial_ap)*2).^2);
...
templates = adj_templates(templates, initial_ap);
...
function [refract_time, refract_index] = refract_period(tInit, xInit, const, duration, initial_ap, sampling_rate)

refract = 0;
i = initial_ap + 1; % 1+ to the initial_ap that was defined
...
function [end_time, end_index, trans_temp] = gen_trans(tInit, xInit, const, duration, initial_ap, sampling_rate, templates)

trans_temp = {};
count = 0;

% Loops through the entirety of the duration if required in steps of 0.2ms
for i = templates.abs_refract_time + initial_ap:1/sampling_rate * 1000:duration
    count = count + 1;
    ...
    if (i > templates.abs_refract_time + initial_ap + 3) ...
...
function templates = adj_templates(templates, initial_ap)

start_ap = 145;     % This is just a variable from the gen_templates
initial_ap_index = find(templates.t == initial_ap);
